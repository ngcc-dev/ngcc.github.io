<!-- synchronized report: sign-21/constant_time.md -->
# Constant-time review — 21 ReSolveD-alpha

Scope: reference implementation, representative instance only: `Implementations/Reference_Implementation/ReSolveD-alpha-384f/SIG_AlgorithmInstance.c`. Other instances and optimized implementations have not been proven source-equivalent. This is a source-level triage, not a constant-time certification.
...
- Branch/loop trace: `Implementations/Reference_Implementation/ReSolveD-alpha-384f/SIG_AlgorithmInstance.c:75` — The wrapper checks public key length. Secret-witness operations in the deeper ReSolveD code need a separate taint trace.
