<!-- synchronized report: kem-38/constant_time.md -->
# Constant-time review — kem-38 UVW-KEM

Scope: representative reference instance `Implementations/Reference_Implementation/UVW-KEM-128/src/KEM_AlgorithmInstance.c`; other levels, optimized copies and compiled machine code are not certified by this source review.
...
- Source trace: `Implementations/Reference_Implementation/UVW-KEM-128/src/KEM_AlgorithmInstance.c:394` — The private-key list decoder returns early on failure; existing `kem-38-2` tracks the resulting explicit failure oracle. This is not a separate timing-only finding.
